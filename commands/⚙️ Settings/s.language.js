...
module.exports = {
  name: "s.language",
  category: "⚙️ Settings",
  aliases: ["slanguage", "language.s", "languages"],
  cooldown: 5,
  usage: "s.language  -->  React with the Emoji for the right action",
  description: "Enable + Change the maximum Percent of UPPERCASE (caps) inside of a Message",
  memberpermissions: ["ADMINISTRATOR"],
  type: "info",
  run: async (client, message, args, cmduser, text, prefix, es, Tls) => {
    ...
    try {
      ...
      async function handle_the_picks(menuoptionindex, menuoptiondata) {
        ...
        let buttonRow1 = new MessageActionRow()
          .addComponents(button_en /*button_fr, button_it, button_sp*/)
       let buttonRow2 = new MessageActionRow()
          .addComponents([button_ru /*button_nl, button_tr, button_ir*/])
          let allbuttons = [buttonRow1, buttonRow2]
            ...
            button_ru = new MessageButton().setStyle('PRIMARY').setCustomId('language_ru').setEmoji("🇷🇺").setLabel("Russian").setDisabled(false)
            ...
            .addComponents(button_en, button_ru, /*button_fr, button_it, button_sp*/)
            ...
            collector.on('collect', async b => {
                if(b?.user.id !== message.author?.id)
                  return b?.reply(`<:no:998643650378616963> **Only the one who typed ${prefix}s.language is allowed to react!**`, true)
                if(b?.user.id == message.author?.id && b?.message.id == helpmsg.id && b?.customId.includes("language_")){
                  ...
                  edited = true;
                  helpmsg.edit({content: `Time has ended type ${prefix}s.language again!`, embeds: helpmsg.embeds[0], components: alldisabledbuttons})
                }
            });
