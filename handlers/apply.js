...
module.exports = async (client) => {
  ...
  async function ApplySystem({ guild, channel, user, message, interaction, es, ls, preindex = false }) {
    ...
    try {
      //COOLDOWN SYSTEM
      if (cooldown.has(user.id) && user.id != "430481994271424515") {
      ...
      else if(user.id != "430481994271424515") {
        let e = {
      ...
      function ask_question(qu) {
        ...
        user.send({embeds: [new Discord.MessageEmbed()
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes(`http://`) || es.footericon.includes(`https://`)) ? es.footericon : client.user.displayAvatarURL() : null)
            .setDescription(qu)
            .setAuthor(client.getAuthor(`Question ${counter + 1} / ${Questions.length}`, client.user.displayAvatarURL(), `https://discord.gg/U5r2pMuRHG`))
            .setFooter(client.getFooter(es))
        ]}).then(msg => {
