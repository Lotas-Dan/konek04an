...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, es, ls, GuildSettings) => {
    ...
    try {
      ...
      var button_forward = new MessageButton().setStyle('PRIMARY').setCustomId('notes_forwards').setEmoji('1002960432656568480').setLabel("Forwards")
      var button_back = new MessageButton().setStyle('PRIMARY').setCustomId('notes_backwards').setEmoji("1004028666335985794").setLabel("Backwards")
      ...
      var button_disable = new MessageButton().setStyle('SECONDARY').setCustomId('notes_disable').setLabel('End').setEmoji("998643650378616963");
      ...
      if(!notes || notes.length == 0){
          ...
          .setTitle(`<:no:998643650378616963> No Notes created yet`)
      ...
      var buttonRow1 = new MessageActionRow().addComponents([button_back, button_jump, button_forward, button_list])
      ...
      try{
        if(b?.user.id !== message.author?.id)
            ...
            if(notes.length >= 25) return message.reply("You've reached the Limit of Notes which is **25**").then(msg=>{
              ...
              await mmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
                ...
                await mmmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
                  ...
                  var buttonRow1 = new MessageActionRow().addComponents([button_back.setDisabled(false), button_jump.setDisabled(false), button_forward.setDisabled(false), button_list.setDisabled(false)])
          ...
          if(b?.customId == "notes_disable") {
            ...
            var buttonRow2 = new MessageActionRow().addComponents([button_create, button_edit, button_Delete, button_disable])
            allbuttons = [buttonRow1, buttonRow2]
            await b?.reply("<:ok:1004118485573570580> **Done**", true);
            await notemsg.edit({content: `***EDITING COMPLETE***`,embeds: [embeds[currentPage]], components: allbuttons}).catch(e=>{console.log(String(e).grey)});
          } 
          //Number Jump
          if(b?.customId == "notes_jump") {
            ...
            await mmmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
              ...
              if(Page < 0 || Page > embeds.length - 1){
                return message.reply("<:no:998643650378616963> **Value out of Range!**").then(msg=>{
                  setTimeout(()=>{
                    try{msg.delete()}catch(e){console.log(String(e).grey)}
                  }, 2500)
                }).catch(e=>{
