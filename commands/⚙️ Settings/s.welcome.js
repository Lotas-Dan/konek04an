...
module.exports = {
  name: "s.welcome",
  category: "⚙️ Settings",
  aliases: ["swelcome", "welcome.s", "welcomes"],
  cooldown: 5,
  usage: "s.welcome --> React with the Emoji for the right action",
    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor('Welcome Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b.png', 'https://discord.gg/U5r2pMuRHG')
        ...
        switch (optionhandletype) {          
              ...
              switch (optionhandletype) {
                ...
                case `Manage the Image`:{
                  ...
                  async function third_layer(){
                    ...
                    let MenuEmbed = new MessageEmbed()
                      .setColor(es.color)
                      .setAuthor(client.getAuthor('Welcome Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b.png', 'https://discord.gg/U5r2pMuRHG'))
                      .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
                    //send the menu msg
                    let menumsg = await message.reply({embeds: [MenuEmbed], components: [new MessageActionRow().addComponents(Selection)]})
                    //Create the collector
          ...
          case "Direct Welcome Messages":{
            ...
            async function second_layer(){
              ...
              let MenuEmbed = new MessageEmbed()
                .setColor(es.color)
                .setAuthor(client.getAuthor('DM - Welcome Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b.png', 'https://discord.gg/U5r2pMuRHG'))
                .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
              //send the menu msg
              let menumsg = await message.reply({embeds: [MenuEmbed], components: [new MessageActionRow().addComponents(Selection)]})
              //Create the collector
                ...
                case `Manage the Image`:{
                  ...
                  async function third_layer(){
                    ...
                    collector.on('collect', async menu => {
                      ...
                      else menu?.reply({content: `https://discord.gg/U5r2pMuRHG You are not allowed to do that! Only: <@${cmduser.id}>`, ephemeral: true});
                    });
                    //Once the Collections ended edit the menu message
                    collector.on('end', collected => {
                      menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<a:yes:996538231644491907> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata){
                    switch (optionhandletype) {
                      ...
                      case `Frame Color`:{
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<a:yes:996538231644491907> **Selected the \`${collected.first().customId}\` Color**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
                        });
                        //Menu Collections
                        collector.on('collect', async button => {
                          if (button?.user.id === cmduser.id) {
          ...
          case `Test Welcome`:{
            ...
            let invitemessage = `Invited by ${member.user.tag ? `**${member.user.tag}**` : `<@${member.user.id}>`}\n<:like:996548970279084083> **X Invites**\n[<:joines:1002958493088759828> X Joins | <:leaves:1002958695770103928> X Leaves | <:no:998643650378616963> X Fakes]`
            if(welcome){
