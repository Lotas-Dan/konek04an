...
module.exports = {
    ...
    usage: "createbackup or createbackup --nomsg",
    type: "server",
    run: async (client, message, args, cmduser, text, prefix, es, ls, GuildSettings) => {

        if (!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)) {
            return message.reply("<:no:998643650378616963> **I am missing the ADMINISTRATOR Permission!**")
        }
        ...
        if (owner.id != cmduser.id) {
            return message.reply(`<:no:998643650378616963> **You need to be the Owner of this Server!**`)
        }

        let backupMessages = 10;
        if (text.includes('--nomsg')) {
            backupMessages = 0;
        }
        ...
        message.channel.send({
            content: `⚠️ **THIS WILL SAVE ALL DATA** ⚠️\n> If there are more then 6 Backups, the oldest one will get deleted!\n> You can also specify the \`--nomsg\` switch if you don't need user messages.\n> Example: \`${prefix}createbackup --nomsg\`\n\n> *Have you tried: \`${prefix}s.autobackup\`, to enable auto backups?*`,
            ...
            collector.on('collect', button => {
                if (button?.user.id === cmduser.id) {
                    collector.stop();
                    button?.reply({ content: `<a:wloading:1008112445576970320> **Now saving the Backup!**\nThis could take up to 2 Minutes (belongs to your data amount)` }).catch(() => null)
                    // Create the backup
                    backup.create(message.guild, {
                        maxMessagesPerChannel: backupMessages,
                        jsonSave: false,
                        saveImages: "base64",
                        jsonBeautify: true,
                        backupMembers: false,
                        ...
                        message.author.send(`<a:yes:996538231644491907> **Backup successfully created.**\n\n**To Load it type:**\n> \`${prefix}loadbackup ${message.guild.id} 1\` ... note 1 is the latest backup, the higher the number the older (5 is the highest) \`${prefix}listbackups ${message.guild.id}\`!\n\n> *Have you tried: \`${prefix}s.autobackup\`, to enable auto backups?*`).catch(e => {
                            message.channel.send(`<a:yes:996538231644491907> **Backup successfully created.**\n\n**To Load it type:**\n> \`${prefix}loadbackup ${message.guild.id} 1\` ... note 1is the latest backup, the higher the number the older (5 is the highest) \`${prefix}listbackups ${message.guild.id}\`!\n\n> *Have you tried: \`${prefix}s.autobackup\`, to enable auto backups?*`);
                        }).then(() => {
                            message.channel.send(`<a:yes:996538231644491907> **Backup successfully created.** The backup ID was sent in dm!\n\n> *Have you tried: \`${prefix}s.autobackup\`, to enable auto backups?*`);
                        })
                    }).catch(e => {
                        console.error(e)
                    })
