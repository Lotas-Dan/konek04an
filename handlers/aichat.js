...
let { MessageEmbed,
    MessageAttachment,
    User, Permissions,
    CommandInteractionOptionResolver } = require('discord.js');
        ...
        try {
            ...
            if (message.channel.id == chatbot) {
                ...
                if (message.attachments.size > 0) {
                    const attachment = new MessageAttachment("https://media.discordapp.net/attachments/912468287496003635/1004038916573249598/unknown.png")
                ...
                try {
                    ...
                    message.channel.send({ content: "<:no:998643650378616963> AI CHAT API IS DOWN" }).catch(() => null)
                }
            }
        } catch (e) {
            console.error(e)
    ...
    async function AFK_SYSTEM_Ping(client, message, guild_settings, setups) {
        try {
            for await (const user of [...message.mentions.users.values()]) {
                ...
                if (d) {
                    await message.reply({ content: `<:Crying:1002956409018789958> **${user.tag}** went AFK <t:${Math.floor(d.stamp / 1000)}:R>!${d.message && d.message.length > 1 ? `\n\n__His Message__\n>>> ${String(d.message).substring(0, 1800).split(`@`).join(`\`@\``)}` : "" }`}).then(msg => {
                        setTimeout(() => {
                            try {
