      ...
      try {
        ...
        if (data) {
          if (data.state != "closed") {
            if(!interaction.replied) await interaction.reply({ content: `<:no:998643650378616963> **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            else await interaction.editReply({ content: `<:no:998643650378616963> **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            return
          }
        }
      } catch {
