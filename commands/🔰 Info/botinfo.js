...
module.exports = {
    ...
    try{
        ...
        let tempmsg = await message.reply({embeds: [new Discord.MessageEmbed().setColor(es.color)
          .setAuthor(client.getAuthor(client.la[ls].cmds.info.botinfo.loading, "https://cdn.discordapp.com/emojis/756773010123522058.gif", "https://discord.gg/U5r2pMuRHG"))
        ]}).catch(console.error)
        ...
        return Promise.all(promises)
          .then(([cluster, shards, guilds, members, ram, rssRam, ping, dbPing, /*players,*/ uptime]) => {
              ...
              for(let index = 0; index<shards.length; index++) {
                ...
                .setAuthor(client.getAuthor(client.user.tag + " Information", es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL(), `https://discord.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=837452033279`)) 
                ...
                .addField(
                  `${uptime[index] < 1000 || members[index] < 1 || guilds[index] < 1 ? `<:offline:1004116932666085506> ` : `<:online:1002966297954504844>`} Cluster #${cluster[index]}${cluster[index] == client.cluster.id ? ` | **Cluster of __this Guild__**` : ``}`, 
                ...
                .setTimestamp()
                .addField(client.la[ls].cmds.info.botinfo.field5.title, handlemsg(client.la[ls].cmds.info.botinfo.field5.value, {invitelink: `https://discord.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=837452033279`}))
