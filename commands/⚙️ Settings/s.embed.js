...
module.exports = {
  name: "s.embed",
  category: "⚙️ Settings",
  aliases: ["sembed", "embed.s", "embeds"],
  cooldown: 5,
  usage: "s.embed  -->  React with the Emoji for the right action",
  description: "Change the Look of your Embeds (Color, Image, Thumbnail, ...)",
  memberpermissions: ["ADMINISTRATOR"],
  type: "info",
  run: async (client, message, args, cmduser, text, prefix, Tes, Tls, GuildSettings) => {
    let es = Tes;
    let ls = Tls;
    ...
    try {
        ...
        collector.on('collect', async button => {
          ...
          else button?.reply({content: `<:no:998643650378616963> You are not allowed to do that! Only: <@${cmduser.id}>`, ephemeral: true});
        });
        //Once the Collections ended edit the menu message
        collector.on('end', collected => {
          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<a:yes:996538231644491907> **Selected the \`${collected.first().customId}\`. Button**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });
