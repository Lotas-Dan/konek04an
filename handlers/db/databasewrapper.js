...
const StandartSchema = new mongoose.Schema({
    ID: {
...
const DatabaseClass = class extends Tinyfy.TypedEmitter {
    ...
    async get(key) {
        ...
        if(this.cache.has(key)){
            ...
            key == "996502122369654944" ? console.log(`${"GET".blue} ${String(this.cache.has(key)).green} for the Key: ${String(key).green} in ${String(this.model.collection.name).green}`.dim) : null
        }
        else {
            key == "996502122369654944" ? console.log(`${"FETCH-DB".blue} ${String(this.cache.has(key)).green} for the Key: ${String(key).green} in ${String(this.model.collection.name).green}`.dim) : null
    ...
    async set(t, e, n = -1) {
        if(this.cache.has(t)) {
            t == "996502122369654944" ? console.log(`${"DELETE".blue} Cache for ${String(t).green} in ${String(this.model.collection.name).green} [db.set()]`.dim) : null;
            this.cache.delete(t);
        }
    ...
    async delete(t) {
        ...
        if(this.cache.has(t)) {
            key == "996502122369654944" ? console.log(`${"DELETE".blue} Cache for ${String(t).green} in ${String(this.model.collection.name).green} [db.delete()]`.dim) : null
            this.cache.delete(t);
        }
    ...
    connect() {
        return new Promise((t, e) => {
            ...
            mongoose.createConnection(this.url, this.options, async (o,l) => {
                ...
                this.cache = new Map(),
                this.emit('ready', this), this.__appyEventsBinding(),
                t(this);
            })
        })
    }
...
function Indexer(i, t = 'JSON') {
    let e = i.model(t, StandartSchema);
...
function Define(i) {
    return Object.defineProperty(i, '_esModule', { value: !0 });
}
