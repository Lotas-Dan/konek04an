...
module.exports = async client => {
    ...
    client.getGuildData = (guildId) => {
        return new Promise((res, rej) => {
            ...
            if (client.guilds.cache.has(guildId)) return res(client.guilds.cache.get(guildId));
    ...
    client.getInvite = async (id) => {
        ...
        let ch = await client.channels.fetch("996510466371231794").catch(() => { })
        if (!ch) return `COULD NOT CREATE INVITE FOR: <#996510466371231794> in **${ch.guild.name}**`
        ...
        if (!inv) return `COULD NOT CREATE INVITE FOR: <#996510466371231794> in **${ch.guild.name}**`
        return `<#996510466371231794> | discord.gg/${inv.code}`
    }
    // on this shard
    client.getUser = (id) => {
    ...
    client.getUserData = (id) => {
        return new Promise(async (res, rej) => {
            ...
            if (user) res(user); else {
                await client.cluster.broadcastEval(async (c, ctx) => c.users.cache.get(ctx) || await c.users.fetch(ctx).catch(() => null), {
                    context: id
    ...
    client.getChannelData = (id) => {
        return new Promise(async (res, rej) => {
            ...
            if (channel) res(channel); else {
                await client.cluster.broadcastEval(async (c, ctx) => c.channels.cache.get(ctx) || await c.channels.fetch(ctx).catch(() => null), {
                    context: id
                }).catch(rej).then(d => res(d.filter(Boolean)[0]))
    ...
    client.getFooter = (es, stringurl = null) => {
        ...
        if (!text || text.length < 1) text = `${client.user.username} | By: LotasDan`;
    ...
    client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
        ...
        if (!name || name.length < 1) name = `${client.user.username} | By: LotasDan`;
        if (!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
        if (!url || url.length < 1) url = `https://discord.gg/U5r2pMuRHG`;
        ...
        //verify the iconURL
        if (!url.startsWith("https://") && !url.startsWith("http://")) url = `https://discord.gg/U5r2pMuRHG`;
        if (!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
        if (![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
        //return the footerobject
    ...
    client.on("guildCreate", async guild => {
        ...
        let embed = new MessageEmbed()
            .setColor("GREEN")
            .setTitle(`<a:join_vc:1004109391387168900> Joined a New Server`)
    ...
    client.on("guildDelete", async guild => {
        ...
        let embed = new MessageEmbed()
            .setColor("RED")
            .setTitle(`<:leaves:1002958695770103928> Left a Server`)
            .addField("Guild Info", `>>> \`\`\`${guild.name} (${guild.id})\`\`\``)
            .addField("Owner Info", `>>> \`\`\`${theowner ? `${theowner.tag} (${theowner.id})` : `${theowner} (${guild.ownerId})`}\`\`\``)
            .addField("Member Count", `>>> \`\`\`${guild.memberCount}\`\`\``)
            .addField("Servers Bot is in", `>>> \`\`\`${client.guilds.cache.size}\`\`\``)
