    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor('Change Status', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/au-kddi/190/purple-heart_1f49c.png', 'https://discord.gg/U5r2pMuRHG')
          ...
          case "Status 1. Text":
            {
              var tempmsg = await message.reply({embeds: [new MessageEmbed()
                ...
                .setColor(es.color)
                .setDescription(`Example: \`${prefix}help | ${client.user.username.split(" ")[0]} | by: lotas-dan\`
              ...
              *Enter the text now!*`).setFooter(client.getFooter(es))
              ...
              await tempmsg.channel.awaitMessages({ filter: m => m.author.id == cmduser.id, 
                ...
                .then(async collected => {
                  ...
                  status.status.status1 = String(msg).substring(0, 128);
              ...
              *Enter the text now!*`).setFooter(client.getFooter(es))
              ...
              await tempmsg.channel.awaitMessages({ filter: m => m.author.id == cmduser.id, 
                ...
                .then(async collected => {
                  ...
                  .replace("{songsplayed}", nFormatter(Math.ceil(client.stats.get("global", "songs") * [...client.guilds.cache.values()].length / 10), 2))
                  newStatusText = String(newStatusText).substring(0, 128);
                  status.status.status2 = String(msg).substring(0, 128);
                  client.user.setActivity(newStatusText, {
                    type: config.status.type,
                    url: config.status.url
                  })
                  fs.writeFile(`./configs/config.json`, JSON.stringify(status, null, 3), (e) => {
                    if (e) {
                      console.log(e.stack ? String(e.stack).dim : String(e).dim);
                      return message.channel.send({embeds: [new MessageEmbed()
                        .setFooter(client.getFooter(es))
          ...
          case "Status Type":
            {
                ...
                async function second_layer(){
                  let menuoptions = [
                    {
                      value: "PLAYING",
                      description: `e.g: Playing ${config.status.status1}`
                    },
                    {
                      value: "WATCHING",
                      description: `e.g: Watching ${config.status.status1}`
                    },
                    {
                      value: "STREAMING",
                      description: `e.g: Streaming ${config.status.status1}`
                    },
                    {
                      value: "LISTENING",
                      description: `e.g: Listening to ${config.status.status1}`
                    },
                    {
                      value: "COMPETING",
                      description: `e.g: Competing ${config.status.status1}`
                    },
                    {
                      value: "Cancel",
                      description: `Cancel and stop the Ai-Chat-Setup!`,
