...
module.exports = {
  ...
  cooldown: 5,
  usage: "s-advertise  -->  Follow the Steps",
  type: "bot",
  ...
  run: async (client, message, args, cmduser, text, prefix, es, ls, GuildSettings) => {
    ...
    try {
      ...
      async function first_layer(){
        ...
        .setColor(es.color)
        .setAuthor(client.getFooter('Advertising Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/prohibited_1f6ab.png',  'https://discord.gg/U5r2pMuRHG'))
        ...
        client.on('interactionCreate', async (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<:no:998643650378616963> You already selected something, this Selection is now disabled!`}, {ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<:no:998643650378616963> You are not allowed to do that! Only: <@${cmduser.id}>`}, {ephermal : true});
          }
        });
      }
      ...
      async function handle_the_picks(menuoptionindex, menuoptiondata) {
        switch (menuoptionindex) {
          case 0:
            {
              let advertisement = require("../../configs/advertisement.json");
              advertisement.adenabled = !advertisement.adenabled;
              fs.writeFile(`./configs/advertisement.json`, JSON.stringify(advertisement, null, 3), (e) => {
                if (e) {
